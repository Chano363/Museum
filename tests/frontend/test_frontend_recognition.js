...
/**
 * 将图像编码为base64格式，模拟前端的图像处理过程
 * @param {string} imagePath - 图像路径
 * @param {number} scale - 缩放比例
 * @param {number} quality - JPEG压缩质量
 * @returns {Promise<string>} - base64编码的图像
 */
async function processImage(imagePath, scale = 0.5, quality = 0.7) {
    try {
        // 读取图像
        const image = await Jimp.read(imagePath);
        
        // 缩放图像
        const width = Math.floor(image.bitmap.width * scale);
        const height = Math.floor(image.bitmap.height * scale);
        await image.resize(width, height);
        
        // 压缩图像
        const buffer = await image.quality(quality * 100).getBufferAsync(Jimp.MIME_JPEG);
...
async function test_frontend_recognition() {
    ...
    const optimization_params = [
        { scale: 0.5, quality: 0.7, description: '默认优化 (50%分辨率, 70%质量)' },
        { scale: 0.4, quality: 0.6, description: '高优化 (40%分辨率, 60%质量)' },
        { scale: 0.6, quality: 0.8, description: '低优化 (60%分辨率, 80%质量)' }
    ];
    
    for (const params of optimization_params) {
        console.log(`\n测试优化参数: ${params.description}`);
        ...
        for (const test_case of TEST_CASES) {
            ...
            try {
                ...
                const base64Image = await processImage(imagePath, params.scale, params.quality);
                ...
                if (detections && detections.length > 0) {
                    ...
                    // 验证是否正确识别
                    const is_correct = test_case.expected_gesture.toLowerCase() in detected_gesture.toLowerCase();
                    if (is_correct) {
                        correct_count++;
    ...
    for (const [key, group] of Object.entries(groupedResults)) {
        ...
        console.log(`优化参数: 缩放=${scale}, 质量=${quality}`);
...
if (require.main === module) {
    ...
    try {
        ...
        console.error('错误: 缺少依赖，请先安装: npm install jimp request');
        process.exit(1);
    }
