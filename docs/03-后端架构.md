...
后端支持通过环境变量配置各项参数，在 `.env` 文件中配置以下变量：
...
|--------|--------|------|
| `PORT` | 5000 | 后端服务端口 |
| `CORS_ORIGINS` | * | CORS 允许的源，多个用逗号分隔 |

### 模型路径配置

| 变量名 | 默认值 | 说明 |
|--------|--------|------|
| `DETECTION_MODEL_PATH` | dynamic_gestures/models/YOLOv10n_hands.onnx | 手部检测模型路径 |
| `CLASSIFICATION_MODEL_PATH` | dynamic_gestures/models/crops_classifier.onnx | 手势分类模型路径 |
| `HAND_LANDMARKER_PATH` | models/hand_landmarker.task | MediaPipe 手部关键点模型路径 |
...
| 变量名 | 默认值 | 说明 |
|--------|--------|------|
| `MP_NUM_HANDS` | 1 | 最大检测手数 |
| `MP_MIN_DETECTION_CONFIDENCE` | 0.5 | 最小检测置信度 |
| `MP_MIN_PRESENCE_CONFIDENCE` | 0.5 | 最小存在置信度 |
| `MP_MIN_TRACKING_CONFIDENCE` | 0.5 | 最小追踪置信度 |

### AI 生图配置

| 变量名 | 默认值 | 说明 |
|--------|--------|------|
| `AI_PROVIDER` | jimeng | AI 生图提供商（即梦 AI） |
| `ARK_API_KEY` | - | 火山引擎 Ark API Key |
| `JIMENG_MODEL` | doubao-seedream-4-5-251128 | 即梦 AI 模型名称 |
...
# 模型路径
DETECTION_MODEL_PATH=dynamic_gestures/models/YOLOv10n_hands.onnx
