...
class Deque:
    ...
    def check_is_action(self, x):
        ...
        if x.position == HandPosition.FIST:
            start_index = self.index_position(HandPosition.FIST)
            if self.check_duration_max(start_index):
                self.action = Event.FIST
                ActionManager().trigger_action(self.action, x)
                self.clear()
                return True
    ...
    def set_hand_position(self, hand: Hand):
        ...
        if hand.gesture in [31, 35, 36]: # [palm, stop, stop_inv]
            ...
            hand.position = HandPosition.FIST
        ...
        elif hand.gesture in [2, 41]: # [hand_left, three_gun]
